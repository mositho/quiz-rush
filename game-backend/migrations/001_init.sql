...
create table if not exists game_sessions (
    id uuid primary key default gen_random_uuid(),
    owner_profile_id uuid references user_profiles(id) on delete
    set null,
        ...
        updated_at timestamptz not null default now(),
        check (
            jsonb_typeof(selected_question_set_ids) = 'array'
        ),
        check (
            jsonb_array_length(selected_question_set_ids) > 0
        ),
        check (
...
create index if not exists idx_user_profiles_keycloak_subject on user_profiles(keycloak_subject);
create index if not exists idx_game_sessions_owner_status on game_sessions(owner_profile_id, status);
create index if not exists idx_game_sessions_save_deadline_at on game_sessions(save_deadline_at);
create index if not exists idx_game_session_questions_session_position on game_session_questions(session_id, position);
create index if not exists idx_game_scores_owner_saved on game_scores(owner_profile_id, is_saved);
create index if not exists idx_game_scores_public_saved_score on game_scores(is_public, is_saved, score desc);
create index if not exists idx_game_scores_configuration_public_saved_score on game_scores(
    configuration_key,
    is_public,
    is_saved,
    score desc
